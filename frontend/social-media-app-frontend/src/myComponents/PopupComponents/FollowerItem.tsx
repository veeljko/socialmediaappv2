...
import { useFollowUserMutation, useIsFollowingQuery, useRemoveFollowerMutation, useUnFollowUserMutation } from "@/services/followerApi";
...
export function FollowerItem({ userId }: { userId: string }) {
  ...
  const isOwnFollowerList = authedUser?.id === profileId;
  const isSelf = authedUser?.id === userId;
  const shouldShowFollowButton = !isOwnFollowerList && !isSelf && isFollowing?.answer === false;
  const shouldShowUnFollowButton = !isOwnFollowerList && !isSelf && isFollowing?.answer === true;
