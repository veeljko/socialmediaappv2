...
export function ReplayDisplay({ comment, className }: { comment: CommentCard; className?: string }) {
  ...
  return <div className={cn("flex gap-2 py-2 border-b border-gray-300", isDeleted && "bg-muted/15", className)}>
    {isDeleted ? (
      ...
      <div className="flex w-full justify-between items-start pt-1">
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1.5 flex items-center gap-5">
            {!isDeleted && (
              <button className={actionButtonClass} onClick={() => setInputReply(!inputReply)}>
                Reply
              </button>
            )}
          </div>
        ...
        {isDeleted && (
          <div className="flex shrink-0 flex-col items-center gap-1 opacity-0" aria-hidden="true">
            ...
            <p className="text-xs font-medium">0 likes</p>
          </div>
        )}
      </div>
